...
- Name: e2e.spec.ts >> Agent Tool Matrix E2E Test Suite >> Pipeline Builder and Saving Workflow
...
Locator: locator('div.rounded-xl').filter({ hasText: 'Git Master' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('div.rounded-xl').filter({ hasText: 'Git Master' }).first()
...
- main:
  - navigation:
    ...
    - button "Toggle theme"
    - button "Create Tool"
  - heading "Select Component Nodes" [level=2]
  - paragraph: Click "Add Step" on any tool card to queue it as a step.
  - textbox "Filter node list..."
  - heading "Active Pipeline Sequence" [level=2]
  - paragraph: Sequence execution operates top-to-bottom.
  - text: "0"
  - heading "Pipeline Queue Empty" [level=3]
  - paragraph: Add tools from the left pane to initialize a custom agent workflow.
...
```ts
  ...
  32  |     // Search for Git Master
  33  |     const searchInput = page.getByPlaceholder('Search Tools, Skills, Connectors, or by #tag');
  34  |     await searchInput.fill('Git Master');
  35  |
  36  |     // Check that Git Master card is visible
  37  |     const card = page.locator('h3', { hasText: 'Git Master' }).first();
  38  |     await expect(card).toBeVisible();
  39  |   });
  40  |
  41  |   test('Selecting a tool card opens detailed view', async ({ page }) => {
  42  |     // Search for Git Master first to make it visible and clickable
  43  |     const searchInput = page.getByPlaceholder('Search Tools, Skills, Connectors, or by #tag');
  44  |     await searchInput.fill('Git Master');
  45  |
  46  |     // Click on Git Master card
  47  |     const card = page.locator('h3', { hasText: 'Git Master' }).first();
  48  |     await card.click();
  49  |
  ...
  74  |
  75  |     // 2. Add Git Master to Pipeline from the side node list
  76  |     const item = page.locator('div.rounded-xl', { hasText: 'Git Master' }).first();
> 77  |     await expect(item).toBeVisible();
  ...
  123 |     // 1. Click Git Master to open details view
  124 |     const card = page.locator('h3', { hasText: 'Git Master' }).first();
