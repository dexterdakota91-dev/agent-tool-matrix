  ...
  switch (method) {
    ...
    case "tools/call":
      try {
        ...
        // Fetch only IDs and titles to minimize memory usage
        const minimalDbTools = await prisma.tool.findMany({
          select: { id: true, title: true }
        });
        const toolMeta = minimalDbTools.find(t => normalizeName(t.title) === name);
        if (!toolMeta) throw new Error("Tool not found");

        const tool = await prisma.tool.findUnique({ where: { id: toolMeta.id } });
    ...
    case "prompts/get":
      try {
        ...
        const allPromptsLight = await prisma.tool.findMany({
          where: { type: "prompt" },
          select: { id: true, title: true }
        });

        const targetPromptMeta = allPromptsLight.find(t => normalizeName(t.title) === name);
        if (!targetPromptMeta) throw new Error("Prompt not found");

        const tool = await prisma.tool.findUnique({
          where: { id: targetPromptMeta.id }
        });
