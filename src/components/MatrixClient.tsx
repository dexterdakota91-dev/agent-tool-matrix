...
export function MatrixClient({ initialData }: MatrixClientProps) {
  // Navigation
  ...
  const [apiKeys, setApiKeys] = useState<DbApiKey[]>(initialData?.apiKeys || []);
  const [loading, setLoading] = useState(!initialData || (initialData.tools.length === 0 && initialData.workflows.length === 0));
  ...
  useEffect(() => {
    if (!initialData || (initialData.tools.length === 0 && initialData.workflows.length === 0)) {
      fetchData();
    }
  }, [initialData]);
